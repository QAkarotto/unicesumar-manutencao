...
| [src/LibrarySystem.java](src/LibrarySystem.java) | [campos com new](src/LibrarySystem.java#L9) | Tight Coupling / No DI | Dependências criadas diretamente com new Controller.BookManager, new Controller.UserManager, new LoanManager etc. | Injeção por construtor e interfaces para desacoplamento. |
| [src/LibrarySystem.java](src/LibrarySystem.java) | [handleRegisterBook()](src/LibrarySystem.java#L81) | Duplicate Code | Validação e normalização repetidas também em Controller.BookManager.registerBook. | Consolidar validação em um único ponto. |
...
| [src/Controller.BookManager.java](src/BookManager.java) | [registerBook(...)](src/BookManager.java#L10) | Long Parameter List | Muitos parâmetros tornam manutenção e evolução mais caras. | Introduzir DTO/entidade Model.Book e construtor com validação. |
| [src/Controller.BookManager.java](src/BookManager.java) | [registerBook(...)](src/BookManager.java#L17) | Validation Smell | Fluxo aceita título em branco por workaround legado, contrariando regra de negócio. | Centralizar validação e falhar com mensagem consistente. |
| [src/Controller.BookManager.java](src/BookManager.java) | [listBooksSimple()](src/BookManager.java#L51) | Edge Case mal tratado | Acesso a temp.get(0) quando lista vazia pode causar falha. | Fazer early return com mensagem de sem registros. |
| [src/Controller.BookManager.java](src/BookManager.java) | [updateAvailableWithLegacyRule(...)](src/BookManager.java#L75) | Long Parameter List / Mixed Responsibilities | Mistura atualização de estado, regras por opCode e logging com vários parâmetros de controle. | Separar por operações de estoque e reduzir parâmetros. |
| [src/Controller.UserManager.java](src/UserManager.java) | [registerUser(...)](src/UserManager.java#L5) | Long Parameter List | Cadastro recebe 7 parâmetros primitivos/strings sem objeto de contexto. | Criar UserData e validação centralizada. |
| [src/Controller.UserManager.java](src/UserManager.java) | [validateUserData(...)](src/UserManager.java#L102) | Duplicate Code | Regras de validação semelhantes às usadas no fluxo de cadastro. | Reutilizar único validador de usuário. |
...
| [src/Util.DataUtil.java](src/DataUtil.java) | [classe utilitaria estatica](src/DataUtil.java#L5) | Utility Class Overuse | Muitos métodos globais estáticos e estado compartilhado (scanner/retryCounter). | Reduzir estado global e separar utilitários por responsabilidade. |
| [src/Util.DataUtil.java](src/DataUtil.java) | [datePlusDaysApprox(...)](src/DataUtil.java#L169) | Primitive Obsession / Regra frágil | Cálculo de data por concatenação de string. | Usar API de data e regra de prazo real. |
...
Hint 5  
Observe [Controller.BookManager.registerBook](src/BookManager.java#L10) em [src/Controller.BookManager.java](src/BookManager.java).  
A assinatura longa sugere oportunidade para encapsular dados de entrada.

Hint 6  
Observe [Controller.BookManager.listBooksSimple](src/BookManager.java#L51) em [src/Controller.BookManager.java](src/BookManager.java).  
